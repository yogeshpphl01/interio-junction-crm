...
export default function Settings() {
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-10 py-6">
      ...
      {creating && (
        <NewUserModal onClose={() => setCreating(false)} onCreated={() => { setCreating(false); load(); }} />
      )}
    </div>
  );
}
    ...
    <div className="fixed inset-0 z-50 bg-ink/30 backdrop-blur-sm flex items-center justify-center p-4" onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="bg-bone-paper border border-edge rounded-md w-full max-w-md">
        ...
        <form onSubmit={submit} className="p-5 space-y-3">
          ...
          <FieldS label="Password (leave blank for default)"><input data-testid="user-password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} className={cls} placeholder="interio2026" /></FieldS>
