// Unit + widget tests for the staff app's shared UI helpers.
// These run with `flutter test` and need no backend, device or credentials.
import 'dart:async';

import 'package:flutter/material.dart';
import 'package:flutter_test/flutter_test.dart';
import 'package:company_app/src/widgets.dart';
...
void main() {
  ...
  testWidgets('EmptyState shows its icon and message', (tester) async {
    await tester.pumpWidget(
      const MaterialApp(
        home: Scaffold(body: EmptyState(icon: Icons.inbox_outlined, text: 'Nothing needs your attention')),
      ),
    );
  ...
  testWidgets('AsyncRefresh shows a spinner, then the loaded data', (tester) async {
    ...
    completer.complete('IJ-2026-014');
    await tester.pumpAndSettle();

    expect(find.byType(CircularProgressIndicator), findsNothing);
    expect(find.text('IJ-2026-014'), findsOneWidget);
  });
